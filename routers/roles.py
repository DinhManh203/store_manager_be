...
async def them_quyen(role_name: str, current_admin: dict = Depends(get_current_admin)):
    ...
    return {"message": f"Quyền '{role_name}' đã được ghi nhận. Lưu ý: cần cập nhật Enum UserRole trong code để áp dụng vĩnh viễn.", "role": role_name}

@router.put("/cap-nhat-quyen/{user_id}")
async def cap_nhat_quyen_nguoi_dung(user_id: str, role: str, current_admin: dict = Depends(get_current_admin)):
    db = get_db()
    ...
    if role not in valid_roles:
        raise HTTPException(status_code=400, detail=f"Quyền không hợp lệ. Các quyền cho phép: {', '.join(valid_roles)}")
