...
async def chinh_sua_nha_cung_cap(
    supplier_id: str,
...
async def xoa_nha_cung_cap(
    ...
    existing_supplier = await db.suppliers.find_one({"_id": ObjectId(supplier_id)})
    if not existing_supplier:
        raise HTTPException(status_code=404, detail="Không tìm thấy nhà cung cấp")

    if bool(existing_supplier.get("is_active", True)):
        raise HTTPException(
            status_code=400,
            detail="Chỉ có thể xóa nhà cung cấp đang ngừng cung cấp",
        )
